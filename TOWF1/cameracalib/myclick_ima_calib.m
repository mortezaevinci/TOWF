...
wintx=calibparams.wintx;
winty=calibparams.winty;
dX=calibparams.dX;
dY=calibparams.dY;
n_sq_x=calibparams.n_sq_x;
n_sq_y=calibparams.n_sq_y;
...
while(rotator<4)
    ...
    image(I);
    colormap(map);
    set(calibparams.figurenumber,'color',[1 1 1]);
   ...
   x= [];y = [];
    ...
    for count = 1:4,
        ...
        if (size(faroutcc,2)<4)|(manualinput==1)
    ...
    if calibparams.manual_squares,
        ...
        if (n_sq_x1~=n_sq_x)|(n_sq_y1~=n_sq_y)|(n_sq_y1~=n_sq_y2)|(n_sq_x1~=n_sq_x2)
            'warning: bad calibration. this has to change to retry getting a new image'
            rotator=rotator+1;
        else
            rotator=5; %good calibration
        end
...
calibparams.x{calibparams.n_img}=x;
calibparams.X{calibparams.n_img}=X;