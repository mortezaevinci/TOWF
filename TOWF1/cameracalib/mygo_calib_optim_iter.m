...
rig3D = 0;
for kk = 1:calibparams.n_img
    
    if is3D(calibparams.X{kk})
        rig3D = 1;
    end
end


if center_optim & (length(1:n_img) < 2) & ~rig3D,
...
if center_optim & (length(1:n_img) < 5) & ~rig3D,
...
if ~exist('fc'),
    ...
    myinit_intrinsic_param; % The right way to go (if quick_init is not active)!
    alpha_smooth = 0.1; % slow convergence
    est_fc = [1;1];
end;
...
while (change > 1e-9)&(iter < MaxIter),
    ...
    N_points_views_active = N_points_views(1:n_img);
    
    JJ3 = sparse([],[],[],15 + 6*n_img,15 + 6*n_img,126*n_img + 225);
    ex3 = zeros(15 + 6*n_img,1);
    ...
    selected_variables = [est_fc;center_optim*ones(2,1);est_alpha;est_dist;zeros(5,1);reshape(ones(6,1)*ones(1,n_img),6*n_img,1)];
    if ~est_aspect_ratio,
...
param_error = zeros(6*n_img+15,1);
