function [x,X,n_sq_x,n_sq_y,ind_orig,ind_x,ind_y,redcenter,goodresults] = myextract_grid(I,wintx,winty,fc,cc,kc,dX,dY,n_sq_x,n_sq_y,figurenumber,xr,yr,click_mode);
...
redthres=135;
redI=((I(:,:,1))>((I(:,:,2)+I(:,:,3))/2+redthres));

redL=bwlabel(redI);
stats=regionprops(redL,'area','centroid');
[~,redind]=max([stats.Area]);
if ~isempty(redind)
redcenter=stats(redind).Centroid;
else
    redcenter=[0 size(I,2)];
end
...
while (rotator<4)
    ...
    faroutcc=myfaroutcorners(I);
        ...
        manualinput=input('manualcorners? 0 or 1=yes ');
    for count = 1:4,
        ...
        dcount=count+rotator;
        if dcount>4
            dcount=dcount-4;
        end
        
        xi=faroutcc(1,dcount);
        yi=faroutcc(2,dcount);
        end
        ...
        yi = xxi(2);
        figure(figurenumber);
        plot(xi+10,yi+10,'+','color',[ 1.000 0.314 0.510 ],'linewidth',2);
       
       
        %text(xi,yi,num2str(dcount),'color',[1 1 1 ]);
...
%%my addition       
%     dist2dist=(x-redcenter(1)).^2+(y-redcenter(2)).^2;
%     [~,redfarind]=max(dist2dist);
%     
%     
%     switch redfarind
%         case 1
...
%my addition ends
    ...
    if (n_sq_x1~=n_sq_x2)|(n_sq_y1~=n_sq_y2)|(n_sq_y~=n_sq_y2)|(n_sq_x~=n_sq_x2)
...
% inside points by the planar homography
