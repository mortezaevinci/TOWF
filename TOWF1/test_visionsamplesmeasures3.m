...
kk0=0;
for ii=1:numel(visionsamples.towelanalysis)
   for jj=1:numel(visionsamples.towelanalysis{ii}.areasample31) 
    kk0=kk0+1;
visionsamplesGA.imm{kk0}=visionsamples.towelanalysis{ii}.areasample41{jj};
visionsamplesGA.edge(kk0)=visionsamples.towelanalysis{ii}.areasample_edge(jj);

end
end


nn=numel(visionsamplesGA.edge);
meas=zeros(26,nn);

for ii=1:nn
    imm0=visionsamplesGA.imm{ii};
    imm0=imm0./max(max(imm0));
    
    %focus measures
  meas(1,ii)=focusmeasure(imm0,'TENV',[]);
  ...
  meas(5,ii)=focusmeasure(imm0,'TENG',[]);
    ...
    meas(12,ii)=mean(mean(cornermetric(imm0)))*10^10; %mean value of the corner metric based on canny
    meas(13,ii)=mean(mean(cornermetric(imm0,'MinimumEigenvalue')))*10^5; %mean value of the corner metric based on canny  
...
results=[visionsamplesGA.edge;
   ...
   %make population
   popn=500000;  %make sure popn is a muliply of 4
  ...
  loopn
      ...
      output0=[output0;exp(((pop(jj,:)*meas(:,ii)))*(-visionsamplesGA.edge(ii)*2+1))];
   ...
   fitout=[];
      ...
      fitout=[fitout;exp(((pop(ffind(1),:)*meas(:,ii)))*(-visionsamplesGA.edge(ii)*2+1))];
          end
             
   
   gaoutcome=[visionsamplesGA.edge;output0';fitout']   %positive output should be edge, and negative output should be non-edge
