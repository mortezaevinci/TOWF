...
for i=1:length(testClassIDs);
    ...
    nn=0;
    if nn
    [dissim, index]=mins(DM(i,:),8);
    %[distNew, index]= min(DM(i,:));   % find Nearest Neighborhood
    winner=votes(trainClassIDs, index,dissim);
    else
        [distNew, index]= min(DM(i,:));   % find Nearest Neighborhood
        winner=trainClassIDs(index);
    end
        
        
    if winner == testClassIDs(i)  % judge whether the nearest one is correctly classified
